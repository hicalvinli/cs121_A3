...
# Code from Calvin Li (44065057)'s Assignment 1 PartA submission
...
def tokenize(content):
    # Initialize list for the tokens
    return re.findall(r"[a-zA-Z\d']+", content)

def remove_stopwords(tokens):
    # Remove all stopwords
    return [token for token in tokens if token not in STOPWORDS]

def computeWordFrequencies(tokens, wordFrequencies):
    # For each token, add it to the dictionary if it has not been seen before
    # Else, increment it
    for token in tokens:
        # Lowercase the token since it is case-insensitive
        token = token.lower()
